...
select UPPER('ЅоЋьЎая Ѕу вј')			-- верхний регистр
select LOWER('ћаЋеЌь ая Ѕу вј')			-- нижний регистр



select @@version						-- верси€ сервера Microsoft SQL Server 2017 (RTM-CU13-OD) (KB4483666) - 14.0.3049.1 (X64)  on Windows Server 2016 Standard 10.0 <X64> (Build 14393: ) (Hypervisor) 
use cherkizovo;							-- подключение к Ѕƒ
kill 800								-- завершить є сессию
...
begin 
			...
			declare @v varchar(10)					-- объ€вление переменных
				set @v='Ћеша'						-- присвоение значений
			print @v								-- результат
			select @v='Ћеша1'						-- присвоение значений в запросе
			select @v								-- результат
